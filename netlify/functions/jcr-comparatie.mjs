...
export default async (req) => {
  ...
  // ——— Cursant: comparația propriului răspuns, doar după deblocarea baremului ———
  if (actiune === "a-mea") {
  ...
  if (actiune === "grup") {
    ...
    const distributieCalificativ = {};
    const omiseCount = {};
    const spearmanValori = [];
    const perCursant = raspunsuri.map((x, i) => {
      ...
      comparaDefecte(x.r.defecte, referinta.defecte).detalii
        .filter((d) => d.status === "omis").forEach((d) => { omiseCount[d.cod] = (omiseCount[d.cod] || 0) + 1; });
      ...
      return {
        ...
        calificativ: x.r.calificativ || "—",
        acordCalificativ: cmp.calificativ.status,
        acordClasament: cmp.clasament.status,
        spearman: cmp.clasament.spearman,
    ...
    return json({
      total: raspunsuri.length, numeDeblocate,
      distributieCalificativ, defecteFrecventOmise, spearmanMediu,
      perCursant, exemplare: s.exemplare || [],
    });
  }
